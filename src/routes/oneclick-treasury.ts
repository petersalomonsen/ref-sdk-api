...
const ONECLICK_API_URL = process.env.ONECLICK_API_URL || "https://1click.chaindefuser.com/v0";
...
function isSputnikDaoAddress(address: string): boolean {
  return address.endsWith(".sputnik-dao.near") || address === "sputnik-dao.near";
}
 ...
 */
router.post("/api/treasury/oneclick-quote", async (req: Request, res: Response) => {
  try {
    const { 
      treasuryDaoID,
      inputToken,
    ...
    if (!treasuryDaoID || !inputToken || !outputToken || !amountIn || !slippageTolerance) {
    ...
    if (!isSputnikDaoAddress(quoteRequest.refundTo) || !isSputnikDaoAddress(quoteRequest.recipient)) {
      return res.status(403).json({
        error: "Security validation failed: refund and recipient must be sputnik-dao.near addresses"
      });
    }
    ...
    if (!response.data || response.data.error) {
      throw new Error(response.data?.error || "Failed to fetch quote from 1Click API");
    }
    ...
    if (axios.isAxiosError(error)) {
      ...
      if (error.response?.status === 401) {
        return res.status(500).json({
          error: "1Click API authentication failed. Please check API key configuration."
        });
      }
